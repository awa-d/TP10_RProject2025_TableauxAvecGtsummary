...
```{r,echo=FALSE,warning=FALSE }
library(gtsummary)
dataset |> tbl_summary(include = c(hmstat))
```
...
```{r chap1tablemerge, echo=TRUE, eval=TRUE, include=TRUE}
...
```{r chap1tablestack, echo=TRUE, eval=TRUE, include=TRUE}
...
#flextable::save_as_html(resultats_flex, path = "tableau_exporté.html") # HTML
...
tbl_summary_gt <- tbl_summary_overall %>%
  as_gt() %>%
  tab_header(
    title = "Statistiques descriptives de l'enquête sur les conditions de vie",
    subtitle = "Analyse du bien-être des ménages au Burkina Faso (2021)"
  ) %>%
  tab_options(
